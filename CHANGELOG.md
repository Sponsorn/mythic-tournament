...
### Improvements
- **Task 13: Layout BT implemented** — Replaced the `public/compositor/layouts/layout-bt.js` stub with a fullscreen best-times panel: a centered 1600px-wide panel (logo + "Best times" header + 4-column grid of dungeon cards showing `duration_ms` formatted as `m:ss`, team, `+level`, and filled stars for upgrades) that fetches `/api/best-times` on mount, every 30s, and on `run:complete`. Empty/error states render an `.lbt-empty` message. Shared `window.Compositor.escapeHtml` is used throughout (no local helper); `unmount` clears the interval and calls `TwitchEmbedManager.detachAll()` defensively. Corresponding `.lbt-*` styles appended to `public/compositor/compositor.css`.
